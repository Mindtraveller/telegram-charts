function createSlider(x, containerElement) {
  ...
  function handleTouchStart(event) {
    ...
    let endHandler = () => {
      persistBorders();
      off(d, 'mousemove', moveHandler);
      off(d, 'touchmove', moveHandler);
      off(d, 'mouseup', endHandler);
      off(d, 'touchend', endHandler);
    }

    on(d, 'mousemove', moveHandler);
    on(d, 'touchmove', moveHandler);
  ...
  function handleSliderMove(event) {
    ...
    let allowedDiff = newLeftBorder < 0 ? diff + newLeftBorder : diff

    if (diff !== allowedDiff) {
      diff = allowedDiff
      newLeftBorder = LEFT_BORDER - diff
    }

    let newRightBorder = RIGHT_BORDER + diff;

    allowedDiff = newRightBorder < 0 ? diff - newRightBorder : diff

    if (diff !== allowedDiff) {
      diff = allowedDiff
      newLeftBorder = LEFT_BORDER - diff
      newRightBorder = RIGHT_BORDER + diff
    }
